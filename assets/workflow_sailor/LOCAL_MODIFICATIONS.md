...
| 2026-08-06 | `scripts/filter_reads.py:320` | Derive `sense` from fragment orientation (`is_reverse` XOR `is_read2`) instead of `read.is_reverse` alone. Upstream lines kept commented as `LEGACY`. | **Changes results for paired-end data.** Paired-end mates are sequenced inward from the two ends of one fragment, so mate 2 always aligns to the opposite strand from mate 1. `sense` selects which conversion `non_target_edit_mismatches()` treats as the target edit vs. a disqualifying mismatch — it is a property of the *fragment*, but was computed from a per-*read* bit that is guaranteed to differ between mates. Both mates of a pair therefore got opposite answers about the same fragment, and every R2 read was scored against the complement of the target edit: genuine edits counted toward `mm_tolerance` while reverse-complement noise passed for free. Measured on a 9-sample PE hg38 run: R1 survived `filter_reads` at 85.5% vs R2 at 90.5% (15,450,494 of each in, 13,203,839 vs 13,979,336 out) — the discarded reads are the multiply-edited ones that carry the STAMP signal. Verified correct for all four proper-pair flags (83/163/99/147) under both strandedness 1 and 2. No-op for single-end (`is_paired` is False), so previously published SE SAILOR runs stay reproducible. |
| 2026-08-06 | `scripts/filter_reads.py:459` + `:487` | `--reverse_stranded` parser: `type=bool` → `default="True", choices=["True","False"]`, with the string→bool conversion done after `parse_args()`. Upstream kept commented as `LEGACY`. | `bool("False")` is `True` — argparse `type=bool` makes *any* non-empty string parse as True, so this flag was always True regardless of the config. Confirmed end-to-end under Snakemake 9.13.4: a JSON config with `"reverse_stranded": false` renders to the shell as `False`, `split_strands.py`'s bash branch handles it correctly, and only this parser mangled it. Forward-stranded (`strandedness 1`) libraries were silently scored against the wrong strand. **Must ship together with the patch above** — the two bugs partially mask each other: previously exactly one mate per pair was mis-scored under either strandedness, but applying the `sense` fix alone would make both mates agree on the *wrong* answer under `strandedness 1`, taking it from 50% to 100% mis-scored. `strandedness 2` is unaffected (True was already the right answer). Pattern matches `rank_edits.py:299-324` for `--keep-100-edited` rather than a `type=` callable, so argparse itself rejects anything other than `True`/`False` (e.g. a config that quotes the value as the JSON string `"true"` now errors loudly instead of silently reading as True). |
